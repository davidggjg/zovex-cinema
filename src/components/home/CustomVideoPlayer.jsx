...
function ForwardIcon() {
  return (
    <svg viewBox="0 0 40 40" width="40" height="40">
      <path
        d="M28 8 L12 8 L12 5 L6 12 L12 19 L12 16 L28 16 Z"
        fill="#cccccc"
        transform="scale(-1,1) translate(-40,0)"
      />
      <rect x="29" y="4" width="5" height="14" fill="#cccccc" rx="1" />
      <text x="20" y="34" textAnchor="middle" fill="#cccccc"
...
export default function CustomVideoPlayer({ movie, onClose }) {
  ...
  useEffect(() => {
    if (showSkipButton) resetHideTimer();
    return () => clearTimeout(hideTimerRef.current);
  }, [showSkipButton, resetHideTimer]);
  ...
  const handleSkipForward = (e) => {
    ...
    try {
      iframeRef.current.contentWindow.postMessage(
        JSON.stringify({ event: "command", func: "remoteControl", args: ["keyCode", 76] }),
        "*"
      );
    } catch (err) {}
    resetHideTimer();
  ...
  return (
    <div
      onMouseMove={showSkipButton ? resetHideTimer : undefined}
      onTouchStart={showSkipButton ? resetHideTimer : undefined}
    ...
    >
      ...
      <div style={{
        ...
        opacity: !showSkipButton || controlsVisible ? 1 : 0,
        pointerEvents: !showSkipButton || controlsVisible ? "auto" : "none",
      }}>
        <p style={{
          color: "#ccc", fontSize: 13, margin: 0, fontFamily: "Arial",
      ...
      {!src ? (
        ...
        <>
          ...
          <iframe
            ref={showSkipButton ? iframeRef : undefined}
            key={src}
            src={src}
