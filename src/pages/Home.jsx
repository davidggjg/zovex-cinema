...
export default function Home() {
  ...
  useEffect(() => {
    if (selectedMovie) document.title = `${selectedMovie.title} | ZOVEX`;else
    if (selectedSeries) document.title = `${selectedSeries} | ZOVEX`;else
  ...
  useEffect(() => {
    if (movieTitle && movies.length > 0) {
      ...
      if (movie) {
        if (movie.series_name) setSelectedSeries(movie.series_name);else
  ...
  const startEdit = (movie) => {
    ...
    if (movie.video_url && movie.video_url.startsWith("http")) {
      ...
      if (type === "youtube") fullUrl = `https://www.youtube.com/watch?v=${vid}`;else
      if (type === "drive") fullUrl = `https://drive.google.com/file/d/${vid}/view`;else
      if (type === "vimeo") fullUrl = `https://vimeo.com/${vid}`;else
      if (type === "dailymotion") fullUrl = `https://www.dailymotion.com/video/${vid}`;else
      if (type === "streamable") fullUrl = `https://streamable.com/${vid}`;else
      if (type === "rumble") fullUrl = `https://rumble.com/embed/${vid}`;else
      if (type === "archive") fullUrl = `https://archive.org/details/${vid}`;else
      if (type === "kan") fullUrl = `https://www.kan.org.il/General/Embed.aspx?id=${vid}`;else
      if (type === "okru") fullUrl = `https://ok.ru/video/${vid}`;else
      if (type === "telegram") fullUrl = `https://t.me/${vid}`;else
      if (type === "jellyfin") fullUrl = vid;else
      if (type === "kaltura") {const parts = vid.split("/");fullUrl = `https://cdnapisec.kaltura.com//p/${parts[0]}/embedPlaykitJs/uiconf_id/${parts[1]}?iframeembed=true&entry_id=${parts[2]}`;} else
      fullUrl = vid;
    }
    ...
    <div style={{ background: "#fff", minHeight: "100vh", direction: "rtl", fontFamily: "Arial, sans-serif" }}>
      ...
      <div style={{ position: "fixed", bottom: 24, left: 16, zIndex: 1000, display: "flex", alignItems: "flex-end", gap: 10 }}>
        <div style={{ background: "#fff", borderRadius: "16px 16px 16px 4px", padding: "10px 14px", boxShadow: "0 4px 18px rgba(0,0,0,.13)", border: "1px solid #eee", maxWidth: 170, direction: "rtl" }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#111", fontFamily: "Arial, sans-serif", marginBottom: 2 }} className="my-5 opacity-100 \u05E8\u05D5\u05E6\u05D9\u05DD \u05E9\u05E0\u05D5\u05E1\u05D9\u05E3 \u05DE\u05E9\u05D5 \u05DC\u05D0\u05EA\u05E8?">רוצה להוסיף סרט? 🎬</div>
...
function BulkImportPanel({ loadMovies, cardStyle, inp, dot, MovieEntity }) {
  ...
  const parseCSV = (text) => {
    ...
    return lines.slice(1).map((line) => {
      ...
      for (let c of line) {
        if (c === '"') inQ = !inQ;else
        if (c === ',' && !inQ) {cols.push(cur.trim());cur = "";} else
        cur += c;
      }
      cols.push(cur.trim());
      const row = {};
