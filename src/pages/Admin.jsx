...
import { Edit2, Trash2, Settings, Plus, Sparkles, ChevronDown, ChevronUp, Image as ImageIcon, X } from "lucide-react";
...
export default function Admin() {
  ...
  const fetchAutoData = async () => {
    if (!tmdbKey) return alert("הזן מפתח TMDB בהגדרות");
    try {
  ...
  const deleteItem = (id) => {
    if (window.confirm("⚠️ האם אתה בטוח שברצונך למחוק?")) {
  ...
  const grouped = useMemo(() => {
    ...
    allItems.forEach(item => {
      if (item.category === "סדרות") {
        const baseName = item.title.split(" - ")[0].split(" פרק ")[0].trim();
    ...
    <div style={adminLayout}>
      <header style={headerStyle}>
        ...
        <div style={{display:'flex', gap:'15px'}}>
          ...
          <Link to="/" style={linkStyle}>אתר</Link>
        </div>
      </header>
          ...
          <>
            <div style={cardStyle}>
              <div style={{display:'flex', justifyContent:'space-between', marginBottom:'15px'}}>
                ...
                <button onClick={fetchAutoData} style={aiBtn}><Sparkles size={16}/> משיכה אוטומטית</button>
              </div>
              ...
              <input placeholder="קישור לוידאו (YouTube/Drive/Rumble)" value={url} onChange={e => setUrl(e.target.value)} style={inStyle} />
              ...
              <div style={{display:'flex', gap:'10px'}}>
                ...
                })} style={btnMain}>{editingId ? "עדכן" : "פרסם"}</button>
                {editingId && <button onClick={resetForm} style={{...btnMain, background:'#ff3b30', width:'100px'}}>ביטול</button>}
              </div>
            </div>
            ...
            <div style={{marginTop:'30px'}}>
              <div style={cardStyle}>
                ...
                <h4 style={{marginTop:'20px'}}>סדרות</h4>
                      ...
                      <div style={epListAdmin}>
                        {eps.map(ep => (
                          <div key={ep.id} style={itemRow}>
                            <span>עונה {ep.metadata?.season} פרק {ep.metadata?.episode}</span>
                            <div>
                              <button onClick={() => {setEditingId(ep.id); setTitle(name); setUrl(ep.video_id); setThumb(ep.thumbnail_url); setType("series"); setSeason(ep.metadata?.season); setEpisode(ep.metadata?.episode); window.scrollTo(0,0);}} style={actionBtn}><Edit2 size={14} color="#0071E3"/></button>
...
const aiBtn = { background: "#F5F5F7", color: "#0071E3", border: "1px solid #0071E3", padding: "8px 15px", borderRadius: "20px", cursor: "pointer", fontWeight: "bold" };
