...
import { Search, Send, Play, ArrowRight, X, Plus, Trash2, Film, Image as ImageIcon, Hash, Loader2, CheckCircle, AlertCircle } from "lucide-react";
...
export default function Home() {
  ...
  // אדמין - הוספת סרט
  const [formData, setFormData] = useState({ title: "", thumbnail_url: "", category: "", video_url: "", description: "" });
  ...
  const handleAddMovie = async () => {
    ...
    try {
      ...
      setFormData({ title: "", thumbnail_url: "", category: "", video_url: "", description: "" });
      setFormStatus({ type: "success", message: "הסרט נוסף בהצלחה!" });
      loadMovies();
    } catch {
      setFormStatus({ type: "error", message: "שגיאה בהוספת הסרט" });
    }
    setAdding(false);
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", background: "#111", fontFamily: "Arial", direction: "rtl" }}>
        <div style={{ background: "#1e1e1e", padding: "40px", borderRadius: "20px", width: "320px", boxShadow: "0 8px 40px rgba(0,0,0,0.5)" }}>
          ...
          <div style={{ marginBottom: "20px" }}>
            <label style={{ color: "#aaa", fontSize: "13px", display: "block", marginBottom: "8px" }}>קוד PIN (מספרים)</label>
            <input
              type="password"
              inputMode="numeric"
              value={pinInput}
              ...
              style={{ width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #333", background: "#2a2a2a", color: "#fff", fontSize: "16px", outline: "none", boxSizing: "border-box" }}
            />
          </div>
          ...
          <div style={{ marginBottom: "25px" }}>
            ...
            <input
              ...
              placeholder="הזן קוד אותיות"
              style={{ width: "100%", padding: "12px", borderRadius: "8px", border: "1px solid #333", background: "#2a2a2a", color: "#fff", fontSize: "16px", outline: "none", boxSizing: "border-box" }}
            />
          </div>

          {loginError && (
            <p style={{ color: "#ff4d4d", textAlign: "center", marginBottom: "15px", fontSize: "14px" }}>{loginError}</p>
          )}

          <button
            onClick={handleAdminLogin}
            style={{ width: "100%", background: "#e50914", color: "#fff", border: "none", padding: "14px", borderRadius: "10px", fontSize: "16px", fontWeight: "bold", cursor: "pointer" }}
          >
            כניסה
          </button>

          <button
            onClick={() => { setShowAdminLogin(false); setPinInput(""); setLetterInput(""); setLoginError(""); }}
            style={{ width: "100%", background: "none", color: "#666", border: "none", padding: "10px", marginTop: "10px", cursor: "pointer", fontSize: "14px" }}
          >
            ביטול
          </button>
        </div>
    ...
    return (
      <div style={{ background: "#f9f9f9", minHeight: "100vh", direction: "rtl", fontFamily: "Arial, sans-serif", padding: "20px" }}>
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", marginBottom: "30px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button onClick={() => setShowAdmin(false)} style={{ background: "#eee", border: "none", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", fontFamily: "Arial" }}>
            יציאה
          </button>
        </div>
        ...
        <div style={{ maxWidth: "900px", margin: "0 auto", display: "grid", gap: "30px" }}>
          ...
          <section style={{ background: "#fff", padding: "25px", borderRadius: "16px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "15px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                <label style={{ fontSize: "14px", fontWeight: "bold", color: "#555" }}>שם הסרט</label>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px", background: "#f5f5f5", padding: "10px 15px", borderRadius: "8px", border: "1px solid #eee" }}>
                  ...
                  <input style={{ background: "none", border: "none", outline: "none", width: "100%", fontSize: "14px" }} value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} placeholder="פעולה, דרמה..." />
                </div>
              </div>

              <div style={{ display: "flex", flexDirection: "column", gap: "8px", gridColumn: "span 2" }}>
                <label style={{ fontSize: "14px", fontWeight: "bold", color: "#555" }}>קישור תמונה</label>
                <div style={{ display: "flex", alignItems: "center", gap: "10px", background: "#f5f5f5", padding: "10px 15px", borderRadius: "8px", border: "1px solid #eee" }}>
                  <ImageIcon size={16} color="#aaa" />
                  <input style={{ background: "none", border: "none", outline: "none", width: "100%", fontSize: "14px" }} value={formData.thumbnail_url} onChange={e => setFormData({ ...formData, thumbnail_url: e.target.value })} placeholder="https://example.com/image.jpg" />
                </div>
              </div>
              ...
              <button
                ...
                disabled={adding}
                style={{ gridColumn: "span 2", background: "#e50914", color: "#fff", border: "none", padding: "14px", borderRadius: "10px", fontSize: "16px", fontWeight: "bold", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}
              >
                {adding ? <Loader2 size={20} /> : "הוסף סרט למערכת"}
              </button>
            </div>
          ...
          {/* רשימת סרטים */}
          <section style={{ background: "#fff", padding: "25px", borderRadius: "16px", boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
            <h2 style={{ fontSize: "20px", marginBottom: "20px" }}>סרטים קיימים ({movies.length})</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {movies.map(movie => (
                <div key={movie.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px", border: "1px solid #eee", borderRadius: "12px" }}>
                  ...
                  >
                    {deleting === movie.id ? <Loader2 size={20} /> : <Trash2 size={20} />}
                  </button>
                </div>
              ))}
            </div>
      ...
      <div style={{ background: "#111", minHeight: "100vh", direction: "rtl", fontFamily: "Arial, sans-serif", color: "#fff" }}>
        ...
        <div style={{ padding: "20px" }}>
          ...
          {selectedMovie.category && (
            <span style={{ background: "#e50914", color: "#fff", padding: "4px 14px", borderRadius: "20px", fontSize: "13px", fontWeight: "bold" }}>{selectedMovie.category}</span>
          )}
          {selectedMovie.description && (
            <p style={{ marginTop: "15px", fontSize: "15px", lineHeight: "1.8", color: "#bbb" }}>{selectedMovie.description}</p>
          )}
          <button
